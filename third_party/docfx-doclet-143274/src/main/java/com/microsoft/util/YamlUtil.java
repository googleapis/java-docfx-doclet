...
public class YamlUtil {
  ...
  /**
   * Same instance of {@link Remark} class reused for better performance according to authors
   * recommendations.
   *
   * <p>It wrapped in ThreadLocal because of its non-thread safe nature
   */
  private static ThreadLocal<Remark> remark =
      new ThreadLocal<>() {
        @Override
        protected Remark initialValue() {
          Options options = Options.github();
          options.fencedCodeBlocksWidth = 3;
          options.ignoredHtmlElements.add(
              IgnoredHtmlElement.create("xref", "uid", "data-throw-if-not-resolved"));
