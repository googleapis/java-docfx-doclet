...
public class YmlFilesBuilder {
  ...
  private final Pattern XREF_LINK_CONTENT_PATTERN =
      Pattern.compile("(?<=<xref uid=\").*?(?=\" .*?>.*?</xref>)");
  private final Pattern XREF_LINK_RESOLVE_PATTERN =
  ...
  void buildFilesForInnerClasses(
      Element element, List<TocItem> listToAddItems, List<MetadataFile> container) {
  ...
  <T extends Element> void populateItemFields(
  ...
  void addOverloadReferences(MetadataFileItem item, MetadataFile classMetadataFile) {
    MetadataFileItem overloadRefItem =
    ...
    classMetadataFiles.forEach(
        classMetadataFile -> {
          ...
          for (MetadataFileItem item : classMetadataFile.getItems()) {
            item.setSummary(
                YamlUtil.convertHtmlToMarkdown(
