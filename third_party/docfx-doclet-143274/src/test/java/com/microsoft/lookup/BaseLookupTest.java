...
public class BaseLookupTest {
  ...
  public void makeTypeShort() {
    ...
    assertThat(
        "Wrong result for inner class",
        baseLookup.makeTypeShort("org.apache.commons.lang3.arch.Processor.Arch"),
        is("Processor.Arch"));
    assertThat(
        "Wrong result for class with generic",
        baseLookup.makeTypeShort("java.util.List<java.lang.String>"),
        is("List<String>"));
    assertThat(
        "Wrong result for inner class with generic",
        baseLookup.makeTypeShort("java.util.List.Custom<java.lang.Some.String>"),
        is("List.Custom<Some.String>"));
    assertThat(
        "Wrong result for inner class with complex generic",
        baseLookup.makeTypeShort("a.b.c.D.E.G<m.n.A.B<c.d.D.G<a.F.Z>>>"),
        is("D.E.G<A.B<D.G<F.Z>>>"));
    assertThat(
        "Wrong result for inner class with generic & inheritance",
        baseLookup.makeTypeShort("a.b.G<? extends a.b.List>"),
        is("G<? extends List>"));
  }
  ...
  public void buildXrefTagWhenLabelPresents() {
    ...
    assertThat(
        ...
        result,
        is(
            "<xref uid=\"Some#signature\" data-throw-if-not-resolved=\"false\">"
                + labelValue
                + "</xref>"));
  }
  ...
  public void replaceLinksAndCodes() {
    ...
    assertThat(
        ...
        is(
            "<xref uid=\"Some#signature\" data-throw-if-not-resolved=\"false\">"
                + "Some#signature</xref><code>Some text content</code>"
                + textTreeContent));
  }
  ...
  public void resolve() {
    ...
    assertThat(
        "Consequence call should return same instance",
        resultForKey1,
        is(consequenceCallResultForKey1));
    assertThat(
        "Resolve for another key should return another instance",
        resultForKey2,
        not((resultForKey1)));
  }
  ...
  public void testExtractMethods() {
    ...
    assertThat(
        "Wrong packageName",
        baseLookup.extractPackageName(element),
        is(lastBuiltItem.getPackageName()));
    assertThat(
        "Wrong fullName", baseLookup.extractFullName(element), is(lastBuiltItem.getFullName()));
    ...
    assertThat(
        "Wrong nameWithType",
        baseLookup.extractNameWithType(element),
        is(lastBuiltItem.getNameWithType()));
    assertThat(
        "Wrong methodContent",
        baseLookup.extractMethodContent(element),
        is(lastBuiltItem.getMethodContent()));
    assertThat(
        "Wrong fieldContent",
        baseLookup.extractFieldContent(element),
        is(lastBuiltItem.getFieldContent()));
    assertThat(
        "Wrong constructorContent",
        baseLookup.extractConstructorContent(element),
        is(lastBuiltItem.getConstructorContent()));
    assertThat(
        "Wrong overload", baseLookup.extractOverload(element), is(lastBuiltItem.getOverload()));
    assertThat(
        "Wrong parameters",
        baseLookup.extractParameters(element),
        is(lastBuiltItem.getParameters()));
    assertThat(
        "Wrong exceptions",
        baseLookup.extractExceptions(element),
        is(lastBuiltItem.getExceptions()));

    assertThat(
        "Wrong return",
        baseLookup.extractReturn(element).getReturnType(),
        is(lastBuiltItem.getReturn().getReturnType()));
    assertThat(
        "Wrong return",
        baseLookup.extractReturn(element).getReturnDescription(),
    ...
    assertThat(
        "Wrong typeParameters",
        baseLookup.extractTypeParameters(element),
        is(lastBuiltItem.getTypeParameters()));
    assertThat(
        "Wrong superclass",
        baseLookup.extractSuperclass(element),
        is(lastBuiltItem.getSuperclass()));
    assertThat(
        "Wrong interfaces",
        baseLookup.extractInterfaces(element),
        is(lastBuiltItem.getInterfaces()));
    assertThat("Wrong tocName", baseLookup.extractTocName(element), is(lastBuiltItem.getTocName()));
    assertThat(
        "Wrong references",
        baseLookup.extractReferences(element),
        is(lastBuiltItem.getReferences()));
  }
  ...
  private ExtendedMetadataFileItem buildExtendedMetadataFileItem(Element element) {
    ...
    result.setParameters(
        Arrays.asList(new MethodParameter("method id", "method type", "method desc")));
