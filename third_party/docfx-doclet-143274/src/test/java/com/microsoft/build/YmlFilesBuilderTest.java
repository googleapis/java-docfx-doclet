...
public class YmlFilesBuilderTest {
  ...
  public void setup() {
    ...
    ymlFilesBuilder =
        new YmlFilesBuilder(
            environment, "./target", new String[] {}, new String[] {}, "google-cloud-product");
  }
  ...
  public void addConstructorsInfoWhenOnlyDefaultConstructor() {
    ...
    assertThat(
        "Wrong file name", container.getFileNameWithPath(), is("output" + File.separator + "name"));
  ...
  private void buildRefItemAndCheckAssertions(
      String initialValue, String expectedUid, String expectedName) {
    MetadataFileItem result = ymlFilesBuilder.buildRefItem(initialValue);

    assertThat("Wrong uid", result.getUid(), is(expectedUid));
    assertThat(
        "Wrong name",
        result.getSpecForJava().iterator().next().getUid(),
        is(RegExUtils.removeAll(expectedUid, "\\[\\]$")));
    assertThat("Wrong name", result.getSpecForJava().iterator().next().getName(), is(expectedName));
    assertThat(
        "Wrong fullName",
        result.getSpecForJava().iterator().next().getFullName(),
        is(RegExUtils.removeAll(expectedUid, "\\[\\]$")));
  }
  ...
  public void determineUidByLinkContent() {
    ...
    assertThat(
        ...
        ymlFilesBuilder.resolveUidByLookup("SomeClass", lookupContext),
        is("a.b.c.SomeClass"));
    assertThat(
        "Wrong result for method",
        ymlFilesBuilder.resolveUidFromLinkContent("SomeClass#someMethod()", lookupContext),
        is("a.b.c.SomeClass.someMethod()"));
    assertThat(
        "Wrong result for method with param",
        ymlFilesBuilder.resolveUidFromLinkContent(
            "SomeClass#someMethod(String param)", lookupContext),
        is("a.b.c.SomeClass.someMethod(String param)"));

    assertThat(
        "Wrong result for unknown class",
        ymlFilesBuilder.resolveUidByLookup("UnknownClass", lookupContext),
        is(""));
    assertThat(
        "Wrong result for null", ymlFilesBuilder.resolveUidByLookup(null, lookupContext), is(""));
    assertThat(
        "Wrong result for whitespace",
        ymlFilesBuilder.resolveUidByLookup(" ", lookupContext),
        is(""));
  }
  ...
  public void splitUidWithGenericsIntoClassNames() {
    List<String> result =
        ymlFilesBuilder.splitUidWithGenericsIntoClassNames("a.b.c.List<df.mn.ClassOne<tr.T>>");

    assertThat("Wrong result list size", result.size(), is(3));
    assertThat(
        "Wrong result list content", result, hasItems("a.b.c.List", "df.mn.ClassOne", "tr.T"));
  }
