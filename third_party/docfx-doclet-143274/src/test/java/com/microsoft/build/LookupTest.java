...
public class LookupTest {
  ...
  public void buildContext() {
    ...
    assertThat(
        "Wrong owner uid",
        context.getOwnerUid(),
        is("Person.setFirstName(String firstName, boolean flag)"));

    for (String localKey : localKeys) {
      assertThat(
          "Context should contain local key=" + localKey, context.containsKey(localKey), is(true));
      assertThat("Wrong value for local key=" + localKey, context.resolve(localKey), is(classUid));
    }

    assertThat(
        "Context should contain global key", context.containsKey(packageNameWithType), is(true));
    assertThat(
        "Context should contain global value as a key", context.containsKey(packageUid), is(true));
    ...
    assertThat(
        "Context shouldn't contain unknown key", context.containsKey("unknown key"), is(false));
  }
